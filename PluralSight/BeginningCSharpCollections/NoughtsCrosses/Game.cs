...
namespace NoughtsCrosses
{
    class Game
    {
        // a Jaggered array (array of arrays)
        ...
        */
        ...
        //a multidemensional array (single array with 2 indices)
        private Square[,] _board = new Square[3, 3];

        public void PlayGame()
        {
            Player player = Player.Crosses;
            bool @continue = true; //just using @ as continue is a c# keyword
            while (@continue)
            {
                DisplayBoard();
                ...
                if (!@continue)
                    return;
                player = 3 - player; //swaps player between x and 0
        ...
        private bool PlayMove(Player player)
        {
            ...
            Console.Write($"{player}: Enter row comm column eg. 3,3 > ");
